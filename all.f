...
: DISTANCE_DETECTION
    1 SEC DELAY
    RED LED OFF YELLOW LED OFF GREEN LED OFF BUZZER LOW
    BEGIN
    DEPTH 4 < WHILE 
		ECHO_PIN IS_ON IF TIME_OUT DELAY THEN TRIGGER
        BEGIN ECHO_PIN IS_ON 0 = WHILE REPEAT NOW
    ...
    DEPTH 4 = IF 
        ...
        SWAP - 154 * 2 / 1 MSEC /
        2DUP < IF DROP ELSE NIP THEN DUP . ." mm "
            DUP 5A < IF RED LED ON BUZZER HIGH THEN 
            DUP 5A C8 WITHIN IF RED LED ON THEN
            DUP C8 190 WITHIN IF YELLOW LED ON THEN
            DUP 190 > IF GREEN LED ON THEN
     ELSE STACK_CLEAR RED LED OFF YELLOW LED OFF GREEN LED OFF THEN ;


: SONAR_CHECK BEGIN 1 SEC DELAY DISTANCE_CHECK BREAK_BUTTON IS_CLICKED UNTIL BREAK_BUTTON CLEAR_EVENT ;
: SONAR_LED BEGIN 1 SEC DELAY DISTANCE_DETECTION BREAK_BUTTON IS_CLICKED UNTIL BREAK_BUTTON CLEAR_EVENT ;

: SECURITY_SYSTEM
	BEGIN 
		PIR DUP MOTION_DETECTED IF CLEAR_EVENT ." Movimento rilevato" CR
			BEGIN PIR IS_ON WHILE DISTANCE_DETECTION REPEAT
...
: RESET STACK_CLEAR GPEDS0 @ 0<> IF
        BUTTON5 BUTTON6 OR BUTTON7 OR CLEAR_EVENT THEN ;
: CHECK
    BEGIN
        RESET ." DIGITA LA SEQUENZA E PREMI INVIO "
        BEGIN
            1 SEC DELAY
